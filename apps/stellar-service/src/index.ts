...
app.post('/fund', (_req: Request, res: Response) => {
  ...
  return res.json({
    status: 'ok',
    message: 'Friendbot request would be sent (testnet).',
  });
});

// ── Send Payment ──────────────────────────────────────────────────────────────
app.post('/send', async (req: Request, res: Response) => {
  const { destination, amount } = req.body as {
    destination?: string;
    amount?: number;
  };
...
app.post('/intent', async (req: Request, res: Response) => {
  ...
  const sourceAccount = await horizonServer.loadAccount(serverKeypair.publicKey());
