...
describe('canTransition', () => {
  it('unbilled → billed is allowed', () => expect(canTransition('unbilled', 'billed').allowed).toBe(true));
  it('billed → paid is allowed', () => expect(canTransition('billed', 'paid').allowed).toBe(true));
  it('billed → denied is allowed', () => expect(canTransition('billed', 'denied').allowed).toBe(true));
  it('denied → resubmitted is allowed', () => expect(canTransition('denied', 'resubmitted').allowed).toBe(true));
  it('resubmitted → billed is allowed', () => expect(canTransition('resubmitted', 'billed').allowed).toBe(true));

  it('unbilled → paid is NOT allowed', () => {
    const r = canTransition('unbilled', 'paid');
    expect(r.allowed).toBe(false);
    expect(r.reason).toContain('"unbilled" → "paid"');
  });
  it('paid → billed is NOT allowed', () => expect(canTransition('paid', 'billed').allowed).toBe(false));
  it('denied → paid is NOT allowed', () => expect(canTransition('denied', 'paid').allowed).toBe(false));
  it('billed → unbilled is NOT allowed', () => expect(canTransition('billed', 'unbilled').allowed).toBe(false));
});
...
describe('buildAgingReport', () => {
  ...
  it('returns empty buckets for no entries', () => {
    buildAgingReport([]).forEach(b => expect(b.encounters).toHaveLength(0));
  });
});
