...
router.get(
  '/:id',
  asyncHandler(async (req: Request, res: Response) => {
    const doc = await PatientModel.findOne({ _id: req.params.id, clinicId: req.user!.clinicId });
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (firstName !== undefined || lastName !== undefined) {
      // Fetch existing scoped to clinic — returns 404 to avoid leaking existence to other clinics
      const existing = await PatientModel.findOne({ _id: req.params.id, clinicId });
      if (!existing)
