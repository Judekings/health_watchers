...
export const MOCK_ENCOUNTERS: EncounterRecord[] = [
  {
    ...
    prescriptions: [{ name: 'Acetaminophen', dose: '500mg', frequency: 'TID (as needed)' }],
    vitals: {
  ...
  },
  {
    ...
    treatmentPlan: 'Continue wound care protocol and resume light activity as tolerated.',
  ...
  },
  {
    ...
    prescriptions: [{ name: 'Naproxen', dose: '250mg', frequency: 'BID with meals' }],
    vitals: {
  ...
  },
  {
    ...
    treatmentPlan: 'Reschedule neurology consultation and maintain headache diary.',
    prescriptions: [{ name: 'Sumatriptan', dose: '50mg', frequency: 'At onset of migraine' }],
    vitals: {
  ...
  return (
    <section className="rounded-xl bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">Encounter List</h2>
          <p className="text-sm text-gray-500">Review and manage clinical encounters</p>
        </div>
        <button
      ...
      <div className="grid gap-3 rounded-lg border border-gray-100 bg-gray-50 p-3 md:grid-cols-4">
        ...
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as 'all' | EncounterStatus)}
          className="rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 outline-none focus:border-blue-400"
        >
              ...
              filteredRows.map((item) => (
                <tr key={item.id} className="hover:bg-blue-50/40">
                  ...
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-gray-500">MRN {item.patientMrn}</div>
                  </td>
                  <td className="px-4 py-3 text-gray-700">{item.doctor}</td>
                  <td className="px-4 py-3 text-gray-700">{formatDateTime(item.encounterAt)}</td>
                  <td className="px-4 py-3">
